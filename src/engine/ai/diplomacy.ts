...
export function evaluateAiTradeNeeds(currentState: GameState, currentOrgId: number): GameState {
    ...
    if(thinkingOrg.resources.rocks < 1000 && thinkingOrg.contextHistory.previousIncome.rocks < 0){
        nextState = sendDiploRequest(nextState, 1, currentOrgId, 'trade',  { send: {credits: 2000, rocks: 0, consumerGoods: 0, gas: 0}, receive: {credits: 0, rocks: 1500, consumerGoods: 0, gas: 0 }});
    }

    if(thinkingOrg.resources.consumerGoods < 1000 && thinkingOrg.contextHistory.previousIncome.consumerGoods < 0){
        nextState = sendDiploRequest(nextState, 1, currentOrgId, 'trade', { send: {credits: 2000, rocks: 0, consumerGoods: 0, gas: 0}, receive: {credits: 0, rocks: 0, consumerGoods: 1500, gas: 0 }});
    }
...
export function evaluateAiRelations(currentState: GameState, currentOrgId: number, targetOrgId: number): DiploType | null {
    ...
    if(currentRelations.status == 'war'){
        if((currentState.intelligence.trueStatus[currentOrgId].militaryStrength < 0.75 * currentState.intelligence.trueStatus[targetOrgId].militaryStrength) || !currentOrg.contextHistory.targetSystems.some(planetoidId => currentState.systems.entities[currentState.planetoids.entities[planetoidId].locationSystemId].ownerNationId === targetOrgId)){
            return 'peace';
        }
    }
    ...
    if(currentRelations.status == 'peace'){
        if(currentState.intelligence.trueStatus[currentOrgId].militaryStrength > 1.2 * currentState.intelligence.trueStatus[targetOrgId].militaryStrength){
            if(currentOrg.contextHistory.targetSystems.some(planetoidId => currentState.systems.entities[currentState.planetoids.entities[planetoidId].locationSystemId].ownerNationId === targetOrgId)){
                return 'war';
            }
        }
    }
