...
import type { GameStoreState, MoveOrderPayload, ShipMoveOrderPayload, GameEvent, ColonizePayload, BuildingClass, DiploType, Pop, PlanetoidIntel, Building } from '../types/gameState';
...
export const useGameStore = create<GameStoreState>((set, get) => {
  ...
  issueMoveOrder: (payload: MoveOrderPayload) => {
    ...
    const newPath = findPath(fleetToUpdate.locationSystemId , targetSystemId, currentState.systems, currentState.lanes);
    set((state) => {
  ...
  buildMilShip: (payload: { locationId: number, shipType: MilShipType }) => {
    set(engineBuildMilShip(get(), 1, payload.locationId, payload.shipType));
  },
  ...
  beginPlanetoidSurvey: (payload: ColonizePayload) => {
    set(beginPlanetoidSurvey(get(), payload));
  },
  ...
  getMilShipById: (id: number) => get().milShips.entities[id],
  ...
  getPopById: (id:number) => get().pops.entities[id],
